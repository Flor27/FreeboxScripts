...
if( scalar @$doneFiles > 0 ) {
	eval { 
		$copiedFiles = lock_retrieve($storageFile);
	} or do {
		$copiedFiles = [];	
	};
	
	@uniques = unique(@$copiedFiles);
	$copiedFiles = \@uniques;	

	if( ! scalar @{$copiedFiles} && scalar @{$doneFiles} > 0) {
		$letsCopy = 1;
		$filesToCopy = $doneFiles;
	} else {

		my %doneFiles = map{$_=>1} @$doneFiles;
		my %copiedFiles = map{$_=>1} @$copiedFiles;

		my @notYesCopiedFIles = grep(!defined $copiedFiles{$_}, @$doneFiles);

		if( scalar @notYesCopiedFIles > 0) {
			$letsCopy = 1;
			$filesToCopy = \@notYesCopiedFIles;
		}		
	}
}
...
sub copyFile {
	...
	print "\n[".localtime. "] Trying to copy '$fileToCopy' <=> '". decode_base64($fileToCopy)."'...";
	eval {
        	$ret = $fbc->api_fs_cp( $subCopyConfig );
        	$taskId = $ret->{id};
	} or do {
        print "\n[".localtime. "] I am sorry Dave, copy hasn't worked with '$fileToCopy' <=> '". decode_base64($fileToCopy)."'...\n";
